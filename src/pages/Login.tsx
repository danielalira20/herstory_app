...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/` // redirige al perfil tras login
      }
    });
  ...
  const handleGithubLogin = async () => {
    ...
    if (error) throw error;
    console.log("🔹 Login con GitHub exitoso:", data);
  } catch (error: any) {
    console.error("❌ Error en login GitHub:", error.message);
    setMensaje("⚠️ No se pudo iniciar sesión con GitHub");
  } 
};
